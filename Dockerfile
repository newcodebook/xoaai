...
RUN --mount=type=cache,id=xoaai-pnpm-store,target=/root/.local/share/pnpm/store \
...
RUN --mount=type=cache,id=xoaai-gomod,target=/go/pkg/mod \
    go mod download
...
RUN --mount=type=cache,id=xoaai-gomod,target=/go/pkg/mod \
    --mount=type=cache,id=xoaai-gobuild,target=/root/.cache/go-build \
...
RUN addgroup -g 1000 xoaai && \
    adduser -u 1000 -G xoaai -s /bin/sh -D xoaai

# Set working directory
WORKDIR /app

# Copy binary/resources with ownership to avoid extra full-layer chown copy
COPY --from=backend-builder --chown=xoaai:xoaai /app/xoaai /app/xoaai
COPY --from=backend-builder --chown=xoaai:xoaai /app/backend/resources /app/resources

# Create data directory
RUN mkdir -p /app/data && chown xoaai:xoaai /app/data

# Copy entrypoint script (fixes volume permissions then drops to xoaai)
...
# Run the application (entrypoint fixes /app/data ownership then execs as xoaai)
