...
const (
	...
	// unsupported 账号的重探间隔倍数：上游不是 xoaai 中转就不会突然长出
	// /v1/xoaai/billing，按常规 interval 重排只会持续占满每周期
...
// UpstreamBillingProbeService discovers a remote XOAAI billing snapshot.
...
func (s *UpstreamBillingProbeService) probeLoadedAccount(ctx context.Context, account *Account, intervalMinutes int) (*UpstreamBillingProbeSnapshot, error) {
	...
	if account.Platform == PlatformOpenAI {
		...
		// 必无 /v1/xoaai/billing；不发请求，直接记 unsupported，避免
	...
	probeURL := buildOpenAIEndpointURL(normalizedBaseURL, "/v1/xoaai/billing")
...
func parseUpstreamBillingProbeResponse(body []byte) (map[string]any, error) {
	...
	if response.Object != "xoaai.key_billing" || response.SchemaVersion != 1 || response.BillingScope != "token" {
...
// in to the upstream billing probe. `/v1/xoaai/billing` is a key-scoped
// xoaai convention shared by the supported API-key platforms (including the
...
// third-party xoaai relay can live under these domains, while custom
// relays (the only targets that can answer /v1/xoaai/billing) always do
...
// serve the same role: official APIs that can never host /v1/xoaai/billing,
...
// 热队列，不再和真正接入 xoaai 的中转账号抢每周期的探测名额。
// 仍按 upstreamBillingProbeMaxDelay 封顶，保证上游后来接入 xoaai 时最迟一天
