...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	debugGatewayBodyEnv                    = "XOAAI_DEBUG_GATEWAY_BODY"
...
type GatewayService struct {
	...
	debugGatewayBodyFile  atomic.Pointer[os.File] // non-nil when XOAAI_DEBUG_GATEWAY_BODY is set
...
func NewGatewayService(
	...
	svc.debugModelRouting.Store(parseDebugEnvBool(os.Getenv("XOAAI_DEBUG_MODEL_ROUTING")))
	svc.debugClaudeMimic.Store(parseDebugEnvBool(os.Getenv("XOAAI_DEBUG_CLAUDE_MIMIC")))
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { //nolint:gosec // G703: path 仅来自启动环境变量 XOAAI_DEBUG_GATEWAY_BODY（运维配置），非请求输入
...
// 启用方式（环境变量）：
//
//	XOAAI_DEBUG_GATEWAY_BODY=1                          # 写入 gateway_debug.log
//	XOAAI_DEBUG_GATEWAY_BODY=/tmp/gateway_debug.log     # 写入指定路径
//
