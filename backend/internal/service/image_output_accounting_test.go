...
func TestOpenAIImageOutputCounter_NestedXOAAI_StandardResponse(t *testing.T) {
	// Simulate what a nested xoaai setup receives
	// When the upstream xoaai returns a standard response, the downstream
	// xoaai receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested xoaai text-only response, got %d", count)
	}
}
