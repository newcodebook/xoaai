...
- `version`：插件自身语义化版本。
- `requires.xoaai`：宿主硬兼容范围。
- `recommended_xoaai_version`：建议宿主版本。
- `tested_xoaai_versions`：发布者真实验证过的版本。
...
签名对象是 `manifest.json` 的精确原始字节。发布者私钥不得进入插件包、源码仓库或 XOAAI 运行环境。部署者只配置 Base64 Ed25519 公钥。
