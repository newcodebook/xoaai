...
func TestPluginsMigrationKeepsAccountSchemaUnchanged(t *testing.T) {
	...
	require.Contains(t, sql, "CREATE TABLE IF NOT EXISTS xoaai_plugin_installations")
	require.Contains(t, sql, "CREATE TABLE IF NOT EXISTS xoaai_plugin_bindings")
	require.Contains(t, sql, "config_encrypted TEXT NOT NULL DEFAULT ''")
	require.Contains(t, sql, "REFERENCES xoaai_plugin_installations(id)")
	require.Contains(t, sql, "CREATE UNIQUE INDEX IF NOT EXISTS idx_xoaai_plugin_bindings_one_enabled_scope")
...
func TestPluginArtifactMigrationSupportsExistingInstallations(t *testing.T) {
	...
	sql := strings.Join(strings.Fields(string(content)), " ")
	require.Contains(t, sql, "ALTER TABLE xoaai_plugin_installations")
