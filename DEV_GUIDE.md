...
| 数据库凭据 | user=`xoaai`, password=`xoaai`, dbname=`xoaai` |
...
psql -U xoaai -h 127.0.0.1 -d xoaai -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER xoaai WITH PASSWORD 'xoaai';
...
```bash
# 连接数据库
psql -U xoaai -h 127.0.0.1 -d xoaai
...
# 执行 SQL 文件
psql -U xoaai -h 127.0.0.1 -d xoaai -f migration.sql
```
...
- [上游仓库](https://github.com/newcodebook/xoaai)
