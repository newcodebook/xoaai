...
XOAAI supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to XOAAI. An explicit empty list
...
```nginx
limit_conn_zone $binary_remote_addr zone=xoaai_conn:20m;
limit_req_zone  $binary_remote_addr zone=xoaai_auth:20m rate=5r/s;
limit_req_zone  $binary_remote_addr zone=xoaai_api:40m rate=30r/s;
...
server {
    ...
    limit_conn xoaai_conn 40;

    location ~ ^/(auth|api/auth)/ {
        limit_req zone=xoaai_auth burst=10 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }

    location ~ ^/(v1/)?(embeddings|alpha/search)$ {
        client_max_body_size 32m;
        limit_req zone=xoaai_api burst=60 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }
...
`gzip_types` and do not use `gzip_types *` for XOAAI. The
...
XOAAI `server.trusted_proxies` with the Caddy address/private subnet so the
