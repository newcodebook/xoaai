#!/bin/bash
#
# XOAAI Installation Script
# XOAAI 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/newcodebook/xoaai/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="newcodebook/xoaai"
INSTALL_DIR="/opt/xoaai"
SERVICE_NAME="xoaai"
SERVICE_USER="xoaai"
CONFIG_DIR="/etc/xoaai"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="XOAAI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 XOAAI 服务："
    ...
    ["not_installed"]="XOAAI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 XOAAI。"
    ...
    ["uninstall_complete"]="XOAAI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 XOAAI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 XOAAI"
    ...
    ["server_config_desc"]="配置 XOAAI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="XOAAI Installation Script"
    ...
    # Completion
    ["install_complete"]="XOAAI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start XOAAI service:"
    ...
    ["not_installed"]="XOAAI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove XOAAI from your system."
    ...
    ["uninstall_complete"]="XOAAI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install XOAAI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove XOAAI"
    ...
    ["server_config_desc"]="Configure XOAAI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/xoaai" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/xoaai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="xoaai_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/xoaai" "$INSTALL_DIR/xoaai"
    chmod +x "$INSTALL_DIR/xoaai"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/xoaai"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/xoaai.service << EOF
[Unit]
Description=XOAAI - AI API Gateway Platform
Documentation=https://github.com/newcodebook/xoaai
...
User=xoaai
Group=xoaai
WorkingDirectory=/opt/xoaai
ExecStart=/opt/xoaai/xoaai
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=xoaai
...
enable_autostart() {
    ...
    if systemctl enable xoaai 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status xoaai"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u xoaai -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart xoaai"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop xoaai"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/xoaai" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet xoaai; then
        print_info "$(msg 'stopping_service')"
        systemctl stop xoaai
    fi

    # Backup current binary
    cp "$INSTALL_DIR/xoaai" "$INSTALL_DIR/xoaai.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/xoaai.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/xoaai"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start xoaai

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: XOAAI must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/xoaai" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="xoaai.backup.${current_version}"
        else
            backup_name="xoaai.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/xoaai" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop xoaai 2>/dev/null || true
    systemctl disable xoaai 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/xoaai.service
    systemctl daemon-reload
