...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/xoaai/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/xoaai"
DATA_DIR="/var/lib/xoaai/datamanagement"
SERVICE_FILE_NAME="xoaai-datamanagementd.service"
...
参数:
  ...
  --source  指定 xoaai 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/xoaai-src
EOF
}
...
if ! id xoaai >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 xoaai..."
  useradd --system --no-create-home --shell /usr/sbin/nologin xoaai
else
  echo "[2/6] 系统用户 xoaai 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R xoaai:xoaai /var/lib/xoaai
chmod 0750 "$DATA_DIR"
...
systemctl enable --now xoaai-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status xoaai-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u xoaai-datamanagementd -f
2. 在 xoaai（容器部署时）挂载 socket:
