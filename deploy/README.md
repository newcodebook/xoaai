# XOAAI Deployment Files

This directory contains files for deploying XOAAI on Linux servers and Apple-silicon Macs.
...
| `xoaai-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete XOAAI, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
curl -sSL https://raw.githubusercontent.com/newcodebook/xoaai/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
- 主进程固定探测 `/tmp/xoaai-datamanagement.sock`
...
# Restart XOAAI only
docker compose -f docker-compose.local.yml restart xoaai
...
| `ADMIN_EMAIL` | No | `admin@xoaai.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf xoaai-complete.tar.gz deployment/

# Transfer to new server
scp xoaai-complete.tar.gz user@new-server:/path/to/destination/
...
XOAAI supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/newcodebook/xoaai/releases)
2. Extract and copy the binary to `/opt/xoaai/`
3. Copy `xoaai.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/xoaai.service
   ```
...
The main config file is at `/etc/xoaai/config.yaml` (created by Setup Wizard).
...
/opt/xoaai/
├── xoaai              # Main binary
├── xoaai.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 xoaai
...
# View detailed logs
docker compose logs --tail=100 xoaai
...
# View recent logs
sudo journalctl -u xoaai -n 50

# Check config file
sudo cat /etc/xoaai/config.yaml
...
XOAAI supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.xoaai.org](https://tls.xoaai.org/)** to get TLS fingerprint information for different devices and browsers.
